...
export const loadCommands = async (commandsPath: string) => {
    ...
    let resolved: Map<string, Command<Bot>> = new Map<string, Command<Bot>>();
    for (const file of files) {
        const res = await discoverFiles<Command<Bot>>(commandsPath, file, [], (file, item) => item.command || item.definition!.name);
        resolved = new Map<string, Command<Bot>>([...resolved, ...res]);
    }
    return resolved;
};
